...
<div class="row">
    <div class="col-md-3 text-center">
        <div class="sticky-top pt-2" style="z-index: 999">
            <div class="">
                @include('members.partials.search', ['size' => ''])
            </div>
    ...
    <div class="col-md-9">
        <form class="form-horizontal" method="POST" action="" id="sell_form" accept-charset="UTF-8" >
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="sticky-top pt-2" style="z-index: 999">
                        <div class="card border-0 bg-dark text-white shadow-lg mb-5">
                            <div class="card-body text-center">
                                <h3 class="sectionHeader mb-0">
                                    {{ __('app.Sell_total') }}: 
                                    {{-- <span id="total_amount">0</span>gr. -  --}}
                                    <span id="total_price">0</span>{{  __('app.Coin') }}
                                </h3>
                                (<span id="total_units">0</span> {{ strtolower( __('app.Products') ) }})
                            </div>
                        </div>

                        <nav>
                            <div class="nav nav-pills" id="nav-tab" role="tablist">
                                <a class="mb-2 nav-item nav-link active" id="nav-prod1-tab" data-toggle="tab" href="#nav-prod1" role="tab" aria-controls="nav-prod1" aria-selected="true">{{ __('app.ProductMain') }}</a>
                                <a class="mb-2 nav-item nav-link" id="nav-prod2-tab" data-toggle="tab" href="#nav-prod2" role="tab" aria-controls="nav-prod2" aria-selected="false">{{ __('app.Bar') }}</a>

                                {{-- <button class="btn btn-warning" type="submit"><i class="fa fa-shopping-cart"></i> {{ __('app.Sell_finish') }}</button> --}} 

                                <button class="mb-2 btn btn-danger" type="button" id="sell_reset"><i class="fa fa-history"></i> {{ __('app.Sell_reset') }}</button>
                        
                                <button id="credit_finish" class="mb-2 btn btn-warning mr-2" type="button"><i class="fa fa-shopping-cart"></i> {{ __('app.Sell_finish_credit') }}</button> 
